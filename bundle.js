...
var list = function list(fiddles) {
	return fiddles.map(function (fiddle, i) {
		var segment = new Element('div').addClass('segment');

		var el = new Element('input', fiddle).attr('type', 'text').render();

		var evalEl = new Element('p').addClass('eval').render();
		...
		return segment.addChild([el, evalEl]).attr('key', i).render();
	});
};

var segments = new Element('div').addChild(list(fiddles)).render();

var popup = new Element('div').addClass('popup').render();
...
var buttons = actions.map(function (action) {
	var title = new Element('h3', action.title).render();

	var description = new Element('p', action.description).render();

	var shortcut = new Element('footer', action.shortcut).render();

	var out = new Element('div').addClass('command').addChild(title, description, shortcut);
